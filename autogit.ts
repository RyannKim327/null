class BTreeNode {
    ...
    insertNonFull(key: number) {
        ...
        if (this.isLeaf) {
            ...
            if (this.children[i + 1].keys.length === 2 * this.t - 1) {
                ...
                // this node is split into two. So, we need to check
                // which of the two children is going to have the new key
    ...
    // in `children[]`. The child will be split into two subtrees.
    splitChild(i: number) {
        const t = this.t;
        ...
        // new key and move a key to this node
